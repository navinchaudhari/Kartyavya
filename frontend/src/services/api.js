...
const AI_REQUEST_TIMEOUT_MS = Number(import.meta.env.VITE_AI_REQUEST_TIMEOUT_MS || 150000);
...
api.interceptors.response.use(
  ...
  (error) => {
    if (error.response?.status === 401 && !window.location.pathname.includes('/login')) {
      clearSession();
      toast.error('Your session expired. Please sign in again.', { toastId: 'session-expired' });
...
export const complaintAPI = {
  ...
  nearby: (latitude, longitude, radiusKm = 1) => api.get('/reports/nearby', { params: { latitude, longitude, radiusKm } }),
  track: (code) => api.get(`/reports/track/${encodeURIComponent(code)}`),
  getAll: (params = {}) => api.get('/admin/reports', { params }).then((response) => ({ ...response, data: response.data.content || [] })),
...
export const officerAPI = {
  ...
  updateStatus: (id, formData) => {
    ...
    const status = displayStatus === 'Completed'
      ? 'RESOLVED'
      : displayStatus === 'In Progress' ? 'IN_PROGRESS' : displayStatus;
...
export const analyticsAPI = {
  ...
  aiHotspots: () => api.get('/analytics/ai-hotspots', { timeout: AI_REQUEST_TIMEOUT_MS }),
  refreshAiHotspots: () => api.post('/analytics/ai-hotspots/refresh', undefined, { timeout: AI_REQUEST_TIMEOUT_MS }),
};
