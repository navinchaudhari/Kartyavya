...
function ManageDepartments() {
  ...
  const disableDepartment = async (item) => {
    ...
    try {
      ...
      toast.error(
        apiErrorMessage(requestError, "Unable to disable department."),
      );
    }
  };
      ...
      <section className="dashboard-header premium-header">
        <div className="container d-flex justify-content-between align-items-center flex-wrap gap-3">
          <div>
            ...
            <p className="mb-0">
              Departments become available immediately for officer allocation
              and routing rules.
            </p>
          </div>
          <button
                ...
                {loading ? (
                  ...
                  items.map((item) => (
                    <tr key={item.departmentId}>
                      ...
                      <td>
                        <span
                          className={`badge ${item.enabled ? "bg-success-subtle text-success" : "bg-secondary-subtle text-secondary"}`}
                        >
