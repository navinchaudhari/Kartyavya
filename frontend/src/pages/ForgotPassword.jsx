...
function ForgotPassword() {
  ...
  const requestOtp = async (event) => {
    ...
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.email))
      return setError("Enter a valid registered email address.");
    setLoading(true);
    try {
      await authAPI.forgotPassword({ email: form.email.trim().toLowerCase() });
      setStep(2);
      setMessage(
        "A six-digit OTP was sent to your registered email. It remains valid for 10 minutes.",
      );
      ...
      <div className="auth-card fade-in-up">
        ...
        {step === 2 && (
          <form onSubmit={verifyOtp}>
            <label className="form-label">Six-digit OTP</label>
            <input
              name="otp"
              inputMode="numeric"
              maxLength="6"
