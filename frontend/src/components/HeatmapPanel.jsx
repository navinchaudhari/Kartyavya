...
function complaintPopup(point) {
  ...
  addTextLine(container, 'Gemini confidence', `${Math.round(point.aiConfidence || 0)}%`);
...
function HeatmapPanel({ points = [], hotspots = [] }) {
  ...
  useEffect(() => {
    ...
    points.forEach((point) => {
      ...
      const fillColor = severity === 'HIGH' ? '#ef4444' : severity === 'MEDIUM' ? '#f59e0b' : '#2563eb';
    ...
    hotspots.forEach((hotspot) => {
      const priorityColor = hotspot.priority === 'CRITICAL'
        ? '#7f1d1d'
        : hotspot.priority === 'HIGH' ? '#dc2626' : hotspot.priority === 'MEDIUM' ? '#d97706' : '#2563eb';
      const circle = L.circle([hotspot.centerLatitude, hotspot.centerLongitude], {
